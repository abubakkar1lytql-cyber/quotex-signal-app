...
# 4. DEEP ANALYSIS & CHART WITH TP/SL
    ...
    with st.spinner("Calculating Targets & Drawing Chart Lines..."):
        ...
        if not data.empty:
            ...
            if isinstance(data_4h.columns, pd.MultiIndex):
                try:
                    data_4h = data_4h.xs(ticker, level=1, axis=1)
                except KeyError:
            ...
            try:
                atr_val = AverageTrueRange(high=high_price, low=low_price, close=close_price, window=14).average_true_range().iloc[-1]
                if pd.isna(atr_val) or atr_val <= 0:
                    atr_val = float(close_price.iloc[-1]) * 0.005
            except Exception:
                atr_val = float(close_price.iloc[-1]) * 0.005
            ...
            if "BTC" in selected_pair or "SOL" in selected_pair:
                pip_unit = 1.0
                sl_pips = round(max(atr_val * 1.5, 50.0))
                tp_pips = round(max(atr_val * 2.5, 100.0))
            elif "OIL" in selected_pair or "GOLD" in selected_pair or "SILVER" in selected_pair:
                pip_unit = 0.1
                sl_pips = round(max((atr_val * 1.5) / pip_unit, 15.0))
                tp_pips = round(max((atr_val * 2.5) / pip_unit, 30.0))
            else:
                pip_unit = 0.01 if "JPY" in selected_pair else 0.0001
                sl_pips = int(max(15, min(round((atr_val * 1.5) / pip_unit), 35)))
                tp_pips = int(max(30, min(round((atr_val * 2.5) / pip_unit), 70)))
            ...
            if ema_20 > ema_50 and 52 < rsi < 70:
                ...
                # If market is slightly bullish/bearish, force calculate default TP/SL levels
                status = "buy" if ema_20 > ema_50 else "sell"
                if status == "buy":
                    tp_price = last_price + (tp_pips * pip_unit)
                    sl_price = last_price - (sl_pips * pip_unit)
                else:
